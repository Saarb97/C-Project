...
LinkedList createLinkedList (PrintFunction PrintFunction,EqualFunction EqualFunction,FreeFunction FreeData ,CopyFunction CopyData )
{
	...
	linkedlist->FreeData = FreeData;
	linkedlist->CopyData = CopyData;
...
status appendNode(LinkedList inputList, Element key) {
	Node node = (struct Node_s *)malloc(sizeof(struct Node_s ));
	if(node == NULL)
		return failure; //in case memory allocation didnt work
	node->data = inputList->CopyData(key); //putting a new copy of the Element key into the node's data
...
status deleteNode(LinkedList inputList, Element key) {
	...
	if(current->data == key) { //checking if desired node is the head of the list
		...
		inputList->FreeData(current->data);
		free(current);
		return success;
	}
